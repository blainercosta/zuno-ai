...
export default function BetaTesterPage() {
  ...
  // Captura ?ref= e ?niche= da URL uma única vez ao montar. O ref precisa
  // sobreviver aos 5 steps (o usuário pode fechar a aba e voltar), por isso
  // vai para sessionStorage em vez de ficar só em memória.
  useEffect(() => {
    ...
    if (ref) {
      try {
        ...
        // storage blocked (private mode/quota): referral is optional, keep going
      }
    }

    const niche = searchParams.get('niche');
    if (niche && (SUBSCRIBER_NICHES as readonly string[]).includes(niche) && niche !== 'Outro') {
      setFormData((prev) => ({ ...prev, nicho: niche }));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Busca quantas pessoas entraram pelo link assim que o referral_code
  // do cadastro atual fica disponível (retorno do saveSubscriber).
  useEffect(() => {
    if (!referralCode) return;

    supabase
      .rpc('subscriber_referral_stats', { code: referralCode })
      .then(({ data, error }) => {
        if (error) {
          console.error('Error fetching subscriber_referral_stats:', error);
          return;
        }
        setReferralCount(data?.[0]?.referrals_count ?? 0);
      });
  }, [referralCode]);
  ...
  const saveSubscriber = async () => {
    ...
    try {
      ...
      const ref = sessionStorage.getItem(REF_STORAGE_KEY) || undefined;
      ...
      <div className="min-h-screen bg-[#09090b] text-white flex flex-col items-center justify-center px-6 py-12">
        <div className="max-w-md w-full">
          ...
          <button
            ...
            className="w-full font-medium py-4 rounded-xl transition-all flex items-center justify-center gap-3 bg-[#25D366] hover:bg-[#20BD5A] text-white"
          >
          ...
          <p className="text-center text-xs text-zinc-500 mt-4">
            Toque para iniciar uma conversa com nosso bot
          </p>

          {referralCode && (
            <ReferralInvite referralCode={referralCode} referralCount={referralCount} />
          )}
        </div>
...
function ReferralInvite({ referralCode, referralCount }: ReferralInviteProps) {
  const [copied, setCopied] = useState(false);
  const referralLink = `https://www.usezuno.app/beta?ref=${referralCode}`;
  ...
  return (
    <div className="mt-6 p-4 rounded-xl bg-zinc-900/50 border border-zinc-800/50">
      <p className="text-sm font-medium text-white mb-1">Convide amigos</p>
      <p className="text-xs text-zinc-500 mb-4">
        Compartilhe seu link e ajude mais gente a entrar no beta
      </p>

      <div className="flex gap-2 mb-3">
        <div className="flex-1 min-w-0 bg-black/40 border border-zinc-800 rounded-lg px-3 py-2.5 text-xs text-zinc-400 truncate">
          {referralLink}
        </div>
        <button
          onClick={handleCopy}
          className="shrink-0 px-3 py-2.5 rounded-lg text-xs font-medium bg-zinc-800 text-zinc-300 hover:bg-zinc-700 transition-colors"
        >
      ...
      <button
        ...
        className="w-full font-medium py-3 rounded-xl transition-all flex items-center justify-center gap-2 bg-[#25D366]/10 hover:bg-[#25D366]/20 text-[#25D366] text-sm"
      >
      ...
      {referralCount > 0 && (
        <p className="text-center text-xs text-zinc-500 mt-3">
          {referralCount} {referralCount === 1 ? 'pessoa entrou' : 'pessoas entraram'} pelo seu link
        </p>
      )}
    </div>
  );
