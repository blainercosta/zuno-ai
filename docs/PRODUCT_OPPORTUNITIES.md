...
|---|--------|-----------|----------------------|-----|
| **F0** | **Link de pagamento em produção está morto.** `bill_aJYE06HEdYEUx2fHd3L0mTNa` retorna "Não encontramos este link". `bill_` é uso único na AbacatePay. | [CheckoutPage.tsx:4](../components/CheckoutPage.tsx#L4) | Conversão paga é zero desde que o link expirou. Todo clique em "Pagar" cai em página de erro. | Link reutilizável no painel + `VITE_ABACATEPAY_PAYMENT_URL` (PR #3). Médio prazo: criar cobrança por cliente via API com email, para o webhook casar sem ambiguidade. |
