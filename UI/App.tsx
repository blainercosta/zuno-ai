...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white dark">
      {/* Mobile Header */}
      <div className="lg:hidden sticky top-0 z-50 bg-[#09090b] border-b border-zinc-800 px-4 py-3">
      ...
      <div className="flex">
        {/* Left Sidebar - Hidden */}
        <aside className={`
          fixed lg:sticky top-0 h-screen bg-[#09090b] border-r border-zinc-800 z-40
        ...
        `} style={{ display: 'none' }}>
          <div className="sticky top-0 h-screen flex flex-col">
            ...
            <div className="flex flex-col gap-[14px] items-center px-2">
              {/* Search */}
              <button className="size-14 flex items-center justify-center rounded-xl hover:bg-zinc-800">
              ...
              >
                <svg className="size-6" fill="none" stroke={currentPage === "home" ? "white" : "#CBD5E1"} viewBox="0 0 24 24">
              ...
              <button 
                ...
                className={`size-14 flex items-center justify-center rounded-xl ${currentPage === "jobs" ? "bg-zinc-800" : "hover:bg-zinc-800"}`}
              >
                <svg className="size-6" fill="none" stroke={currentPage === "jobs" ? "white" : "#CBD5E1"} viewBox="0 0 24 24">
        ...
        {/* Main Content */}
        <main className={`flex-1 min-w-0 ${currentPage === "job-detail" || currentPage === "post-job" ? "" : "p-4 md:p-5 lg:p-5"}`}>
          {currentPage === "news" ? (
            <NewsPage onNewsClick={(id) => console.log("News clicked:", id)} />
          ) : currentPage === "post-job" ? (
            <PostJobPage onBack={() => setCurrentPage("jobs")} />
          ) : currentPage === "job-detail" ? (
            <JobDetailPage onBack={() => setCurrentPage("jobs")} />
          ) : currentPage === "jobs" ? (
            <JobsPage
              onJobClick={() => setCurrentPage("job-detail")}
              onPostJobClick={() => setCurrentPage("post-job")}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-5">
            {[...Array(6).keys()].map((i) => (
              <div key={i} className="group cursor-pointer" onClick={() => setShowProjectModal(true)}>
            ...
            <div className="space-y-0">
              {JOBS.map((job, i) => (
                <div key={i} className="flex items-center gap-3 px-4 py-4 -mx-4 hover:bg-zinc-900 rounded-xl">
                  {job.initials ? (
                    <div className={`size-12 rounded-xl bg-${job.color}-500 flex items-center justify-center shrink-0`}>
            ...
            <div className="space-y-0">
              {USERS.map((user, i) => (
                <div key={i} className="flex items-center gap-3 px-4 py-4 -mx-4 rounded-xl">
                  <div className={`size-10 rounded-full bg-${user.color}-500 flex items-center justify-center shrink-0`}>
